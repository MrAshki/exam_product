...
export function TrueFalseFields({ register, disabled }: QuestionFieldProps) {
  return (
    <fieldset className="space-y-2">
      ...
      <label className="flex items-center gap-2 rounded-md border border-slate-200 px-3 py-2 text-sm text-ink-700">
        <input type="radio" value="true" {...register("correct_answer")} disabled={disabled} />
        درست
      </label>
      <label className="flex items-center gap-2 rounded-md border border-slate-200 px-3 py-2 text-sm text-ink-700">
        <input type="radio" value="false" {...register("correct_answer")} disabled={disabled} />
        غلط
      </label>
    </fieldset>
  );
